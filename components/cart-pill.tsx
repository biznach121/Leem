...
export function CartPill() {
  ...
  return (
    <button
      ...
      className="relative grid h-11 w-10 place-items-center text-[#003a37] transition-transform hover:scale-105"
    >
      <svg
        viewBox="0 0 24 24"
        fill="none"
      ...
      <span className="absolute right-0 top-0 grid h-4 min-w-4 place-items-center rounded-full border border-[#003a37]/55 bg-[#e5f2fe] px-1 text-[10px] font-bold leading-none text-[#003a37]">
        {count}
      </span>
    </button>
  );
...
export function CartPillSkeleton() {
  return (
    <span
      aria-hidden
      className="inline-flex h-11 w-10 items-center justify-center text-[#003a37]"
    />
  );
}
